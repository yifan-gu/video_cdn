...
static size_t get_answer_len(dns_answer_t *aptr) {
    return aptr->name_len // sum up the message length
        + sizeof(aptr->type) + sizeof(aptr->class)
        + sizeof(aptr->ttl) + sizeof(aptr->rdlength) + aptr->rdlength;
}

static size_t get_question_len(dns_question_t *qptr) {
    return qptr->qname_len // sum up the message length
...
static void* decode_question(dns_message_t *m, void *offset) {
    char *i;

    /* get qname_len */
    m->question.qname_len = 1;
    for (i = offset; (*i) != 0; i++) {
    ...
    offset += sizeof(m->question.qclass);

    return offset;
}

static void* decode_answer(dns_message_t *m, void *offset) {
    char *i;
...
static void* encode_question(dns_message_t *m, void *offset) {
    /* encode qname */
    ...
    offset = memcpy(offset, &m->question.qclass, sizeof(m->question.qclass))
        + sizeof(m->question.qclass);
    
    return offset;
}

static void* encode_answer(dns_message_t *m, void *offset) {
    /* encode name */
    ...
    /* rdata */
    offset = memcpy(offset, &m->answer.rdata, sizeof(m->answer.rdata))
        + sizeof(m->answer.rdata);
...
int make_question(dns_message_t *m, const char *node) {
    ...
    m->length = sizeof(dns_header_t) + get_question_len(qptr); // sum up the message length
...
int make_answer(dns_message_t *m, dns_message_t *q,
    ...
    SETFLAG(m->header.flag, 1, 0); // QA
    SETFLAG(m->header.flag, 1, 5); // AA
    ...
    /* copy question */
    memcpy(&m->question, qptr, sizeof(*qptr));
    
    /* update length */
    m->length = sizeof(dns_header_t) + get_answer_len(aptr) + get_question_len(qptr); // sum up the message length
...
ssize_t encode_message(dns_message_t *m, void *buf) {
    ...
    if (mm.header.qdcount > 0) {
        offset = encode_question(&mm, offset);
    }
    
    if (mm.header.ancount > 0) {
        encode_answer(&mm, offset);
    }
...
int decode_message(dns_message_t *m, void *buf, ssize_t len) {
    ...
    if (m->header.qdcount > 0) {
        offset = decode_question(m, offset);
    }
    if (m->header.ancount > 0) { // answer
        decode_answer(m, offset);
    }
